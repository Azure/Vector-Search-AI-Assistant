...
Push-Location $(Join-Path .. charts)
...
if ($charts.Contains("pr") -or  $charts.Contains("*")) {
    ...
    $command = "helm upgrade --install $name-api ./chat-service-web-api -f $valuesFile --set ingress.hosts='{$aksHost}' --set image.repository=$acrLogin/product.api --set image.tag=$tag --set hpa.activated=$autoscale"
...
Write-Host "MS OpenAI Chat API deployed on AKS" -ForegroundColor Yellow