...
if ($charts.Contains("pr") -or  $charts.Contains("*")) {
    ...
    $command = "helm upgrade --install $name-api ./chat-web-app -f $valuesFile --set ingress.hosts='{$aksHost}' --set image.repository=$acrLogin/product.api --set image.tag=$tag --set hpa.activated=$autoscale"
...
Write-Host "MS OpenAI Chat Web App deployed on AKS" -ForegroundColor Yellow